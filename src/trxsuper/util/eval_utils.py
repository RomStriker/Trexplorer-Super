...
def calculate_scores(gt_points, pred_points, gt_radii, pred_radii, threshold=1.5):
...
def get_score_nx(preds, targets, elapsed_time=None, dist=False):
...
def get_stats_message_branch(stats_dict, averages_only=False):
    message = ""
...
def get_stats_message(stats, sample_ids=None, averages_only=False):
    message = ""
    message += "Metrics: \n"
    if not averages_only:
        for i, score in enumerate(stats['scores']):
